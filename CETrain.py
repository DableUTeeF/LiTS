    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for i in range(iterations):
                ...
                ls = 0
                t = targets[i:i + args.batch_mul, :, :, :]
                for height in range(512):
                    for width in range(512):
                        loss = criterion(outputs[:, :, height, width], t[:, :, height, width].view(8).long()) / iterations
                        ls += loss
                ls /= 512*512
                ls.backward()
                train_loss += ls.item() * args.batch_mul
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(val_loader):
                ...
                for i in range(iterations):
                    ...
                    for height in range(512):
                        for width in range(512):
                            loss = criterion(outputs[:, :, height, width],
                                             t[:, :, height, width].view(8).long()) / iterations
                            ls += loss
                    ls /= 512 * 512
                    test_loss += ls.item() * args.batch_mul
                    total += targets.size(0)
